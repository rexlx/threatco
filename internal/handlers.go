...
// OSVScannerOutput mirrors the full schema provided in your out.json capture
		...
		Packages []struct {
			...
			Vulnerabilities []struct {
				...
				DatabaseSpecific struct {
					Severity string `json:"severity"` // Catches strings like "MODERATE", "CRITICAL"
...
// NormalizeSeverity parses numeric CVSS scores or raw strings into standard tiers
func NormalizeSeverity(rawString string, scoreFallback string) string {
	// If a numeric group score fallback exists and no categorical string is found, decode it
	if rawString == "" && scoreFallback != "" {
		var score float64
		fmt.Sscanf(scoreFallback, "%f", &score)
		if score >= 9.0 {
			return "critical"
		} else if score >= 7.0 {
			return "high"
		} else if score >= 4.0 {
			return "warning" // Medium / Moderate
		} else if score > 0.0 {
			return "info" // Low
		}
	}

	switch strings.ToLower(rawString) {
	case "critical":
		return "critical"
	case "high":
		return "high"
	...
	case "low", "info":
		return "info"
	default:
		return "warning" // Default safe fallback
	}
}

// RepoScanRequest defines the JSON payload when a user provides a repository URL instead of uploading files
type RepoScanRequest struct {
	URL string `json:"url"`
}

func (s *Server) ToolsNpmCheckHandler(w http.ResponseWriter, r *http.Request) {
	// 1. Authenticate user session context per ThreatCo conventions
	email, ok := r.Context().Value("email").(string)
	if !ok || email == "" {
		parts := strings.Split(r.Header.Get("Authorization"), ":")
	...
	// Create a unique sandbox workspace for this request execution loop
	tmpDir, err := os.MkdirTemp("", "repo-scan-*")
	if err != nil {
		s.LogError(fmt.Errorf("RepoScan: temp dir creation failed: %w", err))
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}
	defer os.RemoveAll(tmpDir)

	var finalResponse []NpmCheckResult
	var scanTargetDir string
	var executionMode string

	// 2. Route request processing based on Content-Type
	if strings.HasPrefix(r.Header.Get("Content-Type"), "application/json") {
		// --- CASE A: Git Repository URL Scan ---
		executionMode = "Git URL"
		...
		if targetURL == "" {
			http.Error(w, "Repository URL cannot be empty", http.StatusBadRequest)
			return
		}

		// Input Validation: Basic sanity checks to protect backend from unintended routing
		if !strings.HasPrefix(targetURL, "https://") && !strings.HasPrefix(targetURL, "http://") {
			http.Error(w, "Only HTTP/HTTPS repository schemas are allowed", http.StatusBadRequest)
			return
		}
		if strings.Contains(targetURL, " ") || strings.Contains(targetURL, ";") || strings.Contains(targetURL, "&&") {
			http.Error(w, "Malicious parameters detected in URL target", http.StatusBadRequest)
			return
		}

		// Configure clone target destination subfolder
		scanTargetDir = filepath.Join(tmpDir, "cloned-repo")

		// Run shallow clone process safely passing independent string tokens
		cloneCmd := exec.Command("git", "clone", "--depth", "1", targetURL, scanTargetDir)
		if err := cloneCmd.Run(); err != nil {
			s.LogError(fmt.Errorf("RepoScan: git clone failed for %s: %w", targetURL, err))
			http.Error(w, "Failed to clone remote repository. Ensure it is public and accessible.", http.StatusBadRequest)
			return
		}

	} else {
		// --- CASE B: Direct File Upload Scan ---
		executionMode = "Direct Upload"
		if err := r.ParseMultipartForm(10 << 20); err != nil {
			s.LogError(fmt.Errorf("RepoScan: upload parsing error: %w", err))
		...
		if len(files) == 0 {
			http.Error(w, "No files or repository URL provided", http.StatusBadRequest)
			return
		}

		scanTargetDir = tmpDir

		// Write uploaded file tree to temporary workspace directory safely
		for _, fileHeader := range files {
			src, err := fileHeader.Open()
			if err != nil {
				finalResponse = append(finalResponse, NpmCheckResult{FileName: fileHeader.Filename, Error: "Could not open file"})
				continue
			}

			cleanedPath := filepath.Clean(fileHeader.Filename)
			if strings.HasPrefix(cleanedPath, "..") || strings.HasPrefix(cleanedPath, "/") {
				src.Close()
				finalResponse = append(finalResponse, NpmCheckResult{FileName: fileHeader.Filename, Error: "Malicious path traversal block"})
				continue
			}

			destPath := filepath.Join(scanTargetDir, cleanedPath)
			if err := os.MkdirAll(filepath.Dir(destPath), 0755); err != nil {
				src.Close()
				finalResponse = append(finalResponse, NpmCheckResult{FileName: fileHeader.Filename, Error: "Workspace generation failed"})
				continue
			}
			...
			if err != nil {
				src.Close()
				finalResponse = append(finalResponse, NpmCheckResult{FileName: fileHeader.Filename, Error: "Storage write allocation error"})
				continue
			}
			...
			if _, err = io.Copy(dst, src); err != nil {
				dst.Close()
				src.Close()
				finalResponse = append(finalResponse, NpmCheckResult{FileName: fileHeader.Filename, Error: "Write stream disruption"})
				continue
			}
			dst.Close()
			src.Close()
	...
	// 3. Fire recursive osv-scanner across the populated workspace target directory
	var outBuf bytes.Buffer
	cmd := exec.Command("osv-scanner", "-r", "--format=json", scanTargetDir)
	cmd.Stdout = &outBuf
	_ = cmd.Run() // Handle findings parsing dynamically from buffer payload

	// Track findings map keyed by manifest/lockfile paths discovered
	fileResultCache := make(map[string][]NpmMatch)

	var scanResults OSVScannerOutput
	if err := json.Unmarshal(outBuf.Bytes(), &scanResults); err == nil {
		for _, res := range scanResults.Results {
			targetPath := res.Source.Path
			targetPath = strings.TrimPrefix(targetPath, scanTargetDir)
			targetPath = strings.TrimPrefix(targetPath, "/")

			if targetPath == "" {
				targetPath = "root-manifest"
			}

			for _, pkg := range res.Packages {
				// Index group values to quickly correlate vulnerability IDs with max_severity numbers
				vulnScoreMap := make(map[string]string)
				for _, g := range pkg.Groups {
					for _, id := range g.IDs {
						vulnScoreMap[id] = g.MaxSeverity
					}
				}
				...
				for _, vuln := range pkg.Vulnerabilities {
					...
					// Extract accurate classification tags using our normalization helper
					groupScoreFallback := vulnScoreMap[vuln.ID]
					resolvedSeverity := NormalizeSeverity(vuln.DatabaseSpecific.Severity, groupScoreFallback)

					fileResultCache[targetPath] = append(fileResultCache[targetPath], NpmMatch{
						Name:        pkg.Package.Name,
						Description: vuln.ID + ": " + desc,
						Severity:    resolvedSeverity, // Now accurately captures critical, high, warning, info
						Type:        pkg.Package.Ecosystem,
					})
				}
			}
		}
	...
	// 4. Construct response array mapping discovered metrics cleanly
	if executionMode == "Git URL" && len(fileResultCache) == 0 {
		// Ensure clean repositories explicitly return a successful placeholder object
		finalResponse = append(finalResponse, NpmCheckResult{
			FileName: "Repository Root Workspace",
			Matches:  []NpmMatch{},
		})
	} else {
		for fileName, findings := range fileResultCache {
			finalResponse = append(finalResponse, NpmCheckResult{
				FileName: fileName,
				Matches:  findings,
			})
	...
	s.LogInfo(fmt.Sprintf("User %s completed recursive %s security audit using OSV-Scanner", email, executionMode))
	w.Header().Set("Content-Type", "application/json")
...
func (s *Server) ToolsNpmCheckHandlerLegacy(w http.ResponseWriter, r *http.Request) {
