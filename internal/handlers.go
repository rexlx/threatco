...
// OSVScannerOutput captures minimal fields from the JSON schema generated by osv-scanner
...
func (s *Server) ToolsNpmCheckHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal(outBuf.Bytes(), &scanResults); err == nil {
		for _, res := range scanResults.Results {
			// Convert scanner internal container paths back to standard reporting names
			reportingName := res.Source.Path
			reportingName = strings.TrimPrefix(reportingName, scanTargetDir)
			reportingName = strings.TrimPrefix(reportingName, "/")

			if reportingName == "" {
				reportingName = "root-manifest"
			}
			...
			for _, pkg := range res.Packages {
				for _, vuln := range pkg.Vulnerabilities {
					...
					fileResultCache[reportingName] = append(fileResultCache[reportingName], NpmMatch{
						Name:        pkg.Package.Name,
						Description: vuln.ID + ": " + desc,
						Severity:    "warning",
						Type:        pkg.Package.Ecosystem, // Dynamically tracks npm, Go, Cargo, etc.
					})
				}
			}
		}
